...
function showitemNoLoadsChart(){
	...
	var Series = [];
	 $.ajax({  
         type : "post",  
         async : false,
         url : "itemChart/getItemNoLoads?otype="+otype+chartStr,
         data : {},  
...
function ItemnoloadsDatagrid(){
	...
	var column = new Array();
	 ...
	 $("#itemNoLoadsTable").datagrid( {
			fitColumns : true,
			height : $("#body").height() - $("#itemNoLoadsChart").height()-$("#itemNoLoads_btn").height()-40,
			width : $("#body").width(),
			idField : 'id',
			pageSize : 10,
...
function domresize() {
	$("#itemNoLoadsTable").datagrid('resize', {
		height : $("#body").height() - $("#itemNoLoadsChart").height()-$("#itemNoLoads_btn").height()-10,
		width : $("#body").width()
	});
}
