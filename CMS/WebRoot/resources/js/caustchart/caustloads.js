...
function showcaustLoadsChart(){
	...
	var Series = [];
	 $.ajax({  
         type : "post",  
         async : false,
         url : "caustChart/getCaustLoads?otype="+otype+chartStr,
         data : {},  
...
function CaustloadsDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='itemChart/goItemLoads?parent="+result.arys1[m].itemid+"'>"+result.arys1[m].name+"</a>",width:width,halign : "center",align : "left"});
                 }
             }  
         },  
        error : function(errorMsg) {  
	 ...
	 $("#caustLoadsTable").datagrid( {
			fitColumns : true,
			height : $("#body").height() - $("#caustLoadsChart").height()-$("#caustLoads_btn").height()-40,
			width : $("#body").width(),
			idField : 'id',
			pageSize : 10,
...
function domresize() {
	$("#caustLoadsTable").datagrid('resize', {
		height : $("#body").height() - $("#caustLoadsChart").height()-$("#caustLoads_btn").height()-10,
		width : $("#body").width()
	});
}
