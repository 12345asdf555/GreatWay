...
function showCompanynoLoadsChart(){
	...
	var Series = [];
	 $.ajax({  
         type : "post",  
         async : false,
         url : "companyChart/getCompanyNoLoads?otype="+otype+"&parent="+parent+chartStr,
         data : {},  
...
function CompanynoloadsDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='caustChart/goCaustNoLoads?parent="+result.arys1[m].itemid+"'>"+result.arys1[m].name+"</a>",width:width,halign : "center",align : "left"});
                 }
             }  
         },  
        error : function(errorMsg) {  
	 ...
	 $("#companyNoLoadsTable").datagrid( {
			fitColumns : true,
			height : $("#body").height() - $("#companyNoLoadsChart").height()-$("#companyNoLoads_btn").height()-40,
			width : $("#body").width(),
			idField : 'id',
			pageSize : 10,
...
function domresize() {
	$("#companyNoLoadsTable").datagrid('resize', {
		height : $("#body").height() - $("#companyNoLoadsChart").height()-$("#companyNoLoads_btn").height()-10,
		width : $("#body").width()
	});
}
