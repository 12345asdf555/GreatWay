...
function BlocnoloadsDatagrid(){
	...
	var column = new Array();
         ...
         success : function(result) {  
             if (result) {
                 ...
                 for(var m=0;m<result.arys1.length;m++){
                	 column.push({field:"a"+m,title:"<a href='companyChart/goCompanyNoLoads?parent="+result.arys1[m].itemid+"'>"+result.arys1[m].name+"</a>",width:width,halign : "center",align : "left"});
                 }
             }  
         },  
        error : function(errorMsg) {  
	 ...
	 $("#blocNoLoadsTable").datagrid( {
			fitColumns : true,
			height : $("#body").height() - $("#blocNoLoadsChart").height()-$("#blocNoLoads_btn").height()-40,
			width : $("#body").width(),
			idField : 'id',
			pageSize : 10,
...
function domresize() {
	$("#blocNoLoadsTable").datagrid('resize', {
		height : $("#body").height() - $("#blocNoLoadsChart").height()-$("#blocNoLoads_btn").height()-10,
		width : $("#body").width()
	});
}
